#!/usr/bin/env bash
# Run a non-interactive command with encrypted Postiz secrets and scrub literal
# secret values from its output. See postiz-scrubbed-exec.py for limitations.
set -euo pipefail

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
secrets_file="${AIGION_POSTIZ_SECRETS_FILE:-${XDG_CONFIG_HOME:-$HOME/.config}/aigion/postiz.enc.yaml}"
sops_bin="${SOPS_BIN:-sops}"

if [[ $# -eq 0 ]]; then
  echo "usage: $0 COMMAND [ARG...]" >&2
  exit 2
fi
if [[ -t 0 || -t 1 || -t 2 ]]; then
  echo "This wrapper is for non-interactive commands only." >&2
  exit 2
fi
if ! command -v "$sops_bin" >/dev/null 2>&1; then
  echo "sops is not available; set SOPS_BIN if needed." >&2
  exit 1
fi
if [[ ! -f "$secrets_file" ]]; then
  echo "Missing encrypted secrets file: $secrets_file" >&2
  exit 1
fi

temporary_dir="$(mktemp -d)"
trap 'rm -rf "$temporary_dir"' EXIT
temporary_secrets="$temporary_dir/postiz.env"
umask 077
"$sops_bin" --output-type dotenv -d "$secrets_file" > "$temporary_secrets"
python3 "$repo_root/scripts/postiz-scrubbed-exec.py" "$temporary_secrets" -- "$@"
